...
- **Venues** (`data/venues.json`): the live feed carries no stadium and OpenFootball only carries the
  host *city*, so a committed static snapshot supplies the **stadium + city** for all 104 matches.
  ...
  makes the stadium labels and the Seattle home-city accent work regardless of data source.
