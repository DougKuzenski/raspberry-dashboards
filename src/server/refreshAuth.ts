...
// setting REFRESH_TOKEN locks the endpoint down: callers must present the token
// in an `X-Refresh-Token` header or a `?token=` query param. With no token
// configured the endpoint stays open (fine on a trusted single-host LAN).
